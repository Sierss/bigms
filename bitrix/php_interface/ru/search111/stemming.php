...
function stemming_ru($word, $flags = 0)
{
	...
	if(count($aspellResult) && $stemCompleted){
		return $aspellResult[$iStem];
	}
		...
		if(!$aspellFlag){
			...
			$aspellResult =  pspell_suggest($pspell, $word); 

			}

		if(count($aspellResult) && !$aspellFlag){
			$result = array();
			$aspellFlag = true;
			for($i = 0, $cnt = count($aspellResult); $i < $cnt; $i++){
				$result[] = stemming_ru(strtoupper(aspellEncode($aspellResult[$i])));
			}
			$result[] = stemming_ru(strtoupper($word));
