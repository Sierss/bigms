        ...
        if ((($arResult["WF_CURR"] - $arParams["BIG_CATALOG_PROP"]) < $arResult["WF_FULL"]))
        {
            if ($arResult["WF_CURR"] < $arResult["WF_FULL"])
            {
                if ($arResult["WF_NUM"] == 1)
                {
                    ...
                    $savedXML .= '<offers>';
                    foreach ($arResult["OFFER"] as $arOffer):
                        $savedXML .= '<offer type="artist.title" id="' . $arOffer["ID"] . '" available="' . $arOffer["AVAIBLE"] . '">';
                        ...
                        if ($arOffer["DISPLAY_PROPERTIES"][$arParams["STARRING"]]["DISPLAY_VALUE"]):
                            $savedXML .= '<starring>' . $arOffer["DISPLAY_PROPERTIES"][$arParams["STARRING"]]["DISPLAY_VALUE"] . '</starring>';
                        endif;
                        if ($arOffer["DISPLAY_PROPERTIES"][$arParams["DIRECTOR"]]["DISPLAY_VALUE"]):
                            $savedXML .= '<director>' . $arOffer["DISPLAY_PROPERTIES"][$arParams["DIRECTOR"]]["DISPLAY_VALUE"] . '</director>';
                        endif;
                        if ($arOffer["DISPLAY_PROPERTIES"][$arParams["ORIGINALNAME"]]["DISPLAY_VALUE"]):
                            $savedXML .= '<originalName>' . $arOffer["DISPLAY_PROPERTIES"][$arParams["ORIGINALNAME"]]["DISPLAY_VALUE"] . '</originalName>';
                        endif;
