		...
		<div class="bx_catalog_item_controls">
			...
			if ($arItem['CAN_BUY'])
			{
				...
				<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
					...
					<div class="byclick">
						<a href="#" data-id="<?=$arItem["ID"]?>" data-iblock="<?=$arItem["IBLOCK_ID"]?>" class="buy_one_button" onclick="yaCounter31721621.reachGoal('catalog-oneclickbuy-button');">купить в 1 клик</a>
                        <?
                        if ('Y' == $arParams['USE_PRODUCT_QUANTITY'])
                        {
                            ?>
...
<?
		...
		$arJSParams = array(
			...
			'PRODUCT' => array(
				...
				'NAME' => $productTitle,
				'PICT' => $PICT, //('Y' == $arItem['SECOND_PICT'] ? $arItem['PREVIEW_PICTURE_SECOND'] : $arItem['PREVIEW_PICTURE'])
		...
		{
			preg_match('/\/catalog\/[^\/]+\//', $arItem['DETAIL_PAGE_URL'], $matches);
			$beginUrl = $matches[0];
			$url = $beginUrl.'compare/?'.$arParams["ACTION_VARIABLE"]."=ADD_TO_COMPARE_LIST&".$arParams["PRODUCT_ID_VARIABLE"]."=#ID#";
			preg_match('/id=([0-9]+)/', $arItem['COMPARE_URL'], $matches);
			$url = str_replace('#ID#', $matches[1], $url);
			$arJSParams['COMPARE'] = array(
				'COMPARE_URL_TEMPLATE' => $url, //$arResult['~COMPARE_URL_TEMPLATE'],
				'COMPARE_PATH' => $beginUrl.'compare/'
			);
		}
